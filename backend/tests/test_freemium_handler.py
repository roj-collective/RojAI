...
from web_handler import handler  # noqa: E402
...
def _make_cognito_event(method="POST", body="", path="/web/generate-listing"):
...
class TestEmergencySwitch:
    ...
    def test_allows_when_generation_enabled(self):
        with patch.dict(os.environ, {"GENERATION_ENABLED": "true"}):
            with patch("web_handler.check_rate_limit"):
                with patch("web_handler.reserve_generation", return_value={"generationCount": 1, "plan": "free", "monthlyLimit": 5, "resetDate": "2026-08-01"}):
                    with patch("web_handler.complete_generation"):
                        result = handler(
                            _make_cognito_event(body=_valid_body()), None
                        )
...
class TestCognitoAuth:
    def test_extracts_user_id_from_jwt_claims(self):
        with patch("web_handler.check_rate_limit"):
            with patch("web_handler.reserve_generation", return_value={"generationCount": 1, "plan": "free", "monthlyLimit": 5, "resetDate": "2026-08-01"}) as mock_reserve:
                with patch("web_handler.complete_generation"):
                    result = handler(
                        _make_cognito_event(body=_valid_body()), None
                    )
    ...
    def test_returns_401_without_auth(self):
        ...
        event = {
            "requestContext": {
                "http": {"method": "POST", "path": "/web/generate-listing"},
                "requestId": "test-no-auth",
            },
            "headers": {"content-type": "application/json"},
            "rawPath": "/web/generate-listing",
            "body": _valid_body(),
        }
...
class TestRateLimitHandler:
    def test_returns_429_when_rate_limited(self):
        ...
        with patch("web_handler.check_rate_limit", side_effect=RateLimitExceeded(5, 30)):
            result = handler(
                _make_cognito_event(body=_valid_body()), None
            )
...
class TestUsageLimitHandler:
    def test_returns_429_when_monthly_limit_reached(self):
        with patch("web_handler.check_rate_limit"):
            with patch(
                "web_handler.reserve_generation",
    ...
    def test_returns_409_on_duplicate_request_without_cache(self):
        ...
        with patch("web_handler.check_rate_limit"):
            with patch(
                ...
                with patch("web_handler.check_idempotency", return_value=("completed", {"status": "completed"})):
                    result = handler(
                        _make_cognito_event(body=_valid_body()), None
                    )
...
class TestReleaseOnFailure:
    def test_releases_reservation_on_bedrock_error(self):
        ...
        with patch("web_handler.check_rate_limit"):
            with patch("web_handler.reserve_generation", return_value={"generationCount": 1, "plan": "free", "monthlyLimit": 5, "resetDate": "2026-08-01"}):
                with patch("web_handler.generate_mock_listing", side_effect=BedrockError("fail")):
                    with patch("web_handler.release_generation") as mock_release:
                        result = handler(
                            _make_cognito_event(body=_valid_body()), None
                        )
...
class TestUsageRoute:
    def test_returns_usage_for_authenticated_user(self):
        with patch("web_handler.get_usage", return_value={
            "plan": "free",
            "generationCount": 3,
            "monthlyLimit": 5,
            "regenerationLimit": 1,
            ...
            event = _make_cognito_event(method="GET", path="/web/usage")
            result = handler(event, None)
            assert result["statusCode"] == 200
            body = json.loads(result["body"])
    ...
    def test_returns_401_without_auth(self):
        event = {
            "requestContext": {
                "http": {"method": "GET", "path": "/web/usage"},
                "requestId": "test-no-auth",
            },
            "headers": {},
            "rawPath": "/web/usage",
    ...
    def test_returns_cached_response_for_completed_duplicate(self):
        ...
        with patch("web_handler.check_rate_limit"):
            with patch(
                ...
                with patch("web_handler.check_idempotency", return_value=(
                    "completed",
...
class TestStaleReservationReuse:
    def test_stale_reservation_does_not_double_increment(self):
        ...
        with patch("web_handler.check_rate_limit"):
            with patch("web_handler.reserve_generation", return_value={
    ...
    def test_active_reservation_returns_409_retryable(self):
        ...
        with patch("web_handler.check_rate_limit"):
            with patch("web_handler.reserve_generation", side_effect=ActiveReservation("key")):
                result = handler(
                    _make_cognito_event(body=_valid_body()), None
                )
                assert result["statusCode"] == 409
